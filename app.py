...
DB_SERVER = os.environ.get('DB_SERVER', 'weatherdb.database.windows.net')
    ...
    try:
        ...
        for row in cursor.fetchall():
            stats.append({
                ...
                "avg_wind_speed": float(row[5]) if row[5] else None,
                "dominant_condition": row[6],
                "stat_date": row[7].isoformat() if row[7] else None
            })
